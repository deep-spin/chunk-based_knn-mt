...
def main(args, override_args=None):
    ...
    dstore_keys = np.memmap(args.dstore_mmap + '/on_the_fly_keys.npy', dtype=np.float32, mode='w+',shape=(args.dstore_size, args.decoder_embed_dim))
    dstore_vals = np.memmap(args.dstore_mmap + '/on_the_fly_vals.npy', dtype=np.int, mode='w+',shape=(args.dstore_size, args.chunk_size))
    keys_vals_map = []
    
    if args.save_decoder_states:
        if args.pca_decoder_states>0:
            dstore_decoder_states = np.memmap(args.dstore_mmap + '/on_the_fly_decoder_states_pca_'+str(args.pca_decoder_states), dtype=np.float16, mode='w+',shape=(int(args.dstore_size*3), args.decoder_embed_dim))
        else:
            dstore_decoder_states = np.memmap(args.dstore_mmap + '/on_the_fly_decoder_states', dtype=np.float16, mode='w+',shape=(int(args.dstore_size*3), args.decoder_embed_dim))
    ...
    for subset in args.valid_subset.split(","):
        ...
        with torch.no_grad():
            ...
            for i, sample in enumerate(progress):
                ...
                keys_vals_map.append(reduce_size)

                with open(args.dstore_mmap + '/on_the_fly_key_vals_map.pkl', 'wb') as f:
                    pickle.dump(keys_vals_map, f)
            ...
            if args.save_decoder_states and args.chunk_size>1:
                ...
                with open(args.dstore_mmap+'/on_the_fly_decoder_states_map.pkl','wb') as f:
                    pickle.dump(decoder_states_map,f)

                if args.pca_decoder_states>0:
                    pca = pickle.load(open(args.dstore_mmap + 'pca_' + str(args.pca_decoder_states),'rb'))
                    print('fit')
                    ...
                    print('save')
                    torch.save(pca_decoder_states, args.dstore_mmap + '/on_the_fly_decoder_states_pca_'+str(args.pca_decoder_states))
                else:
                    for b in range(0,decoder_states_size,100000):
                        if b==0:
                    ...
                    torch.save(save_decoder_states, args.dstore_mmap + '/on_the_fly_decoder_states')
