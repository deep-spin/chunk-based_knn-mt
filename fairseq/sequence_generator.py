...
class SequenceGenerator(nn.Module):
    ...
    def _generate(
        ...
        new_sent=True
        self.idx += int(tokens.size(0) / beam_size)
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            with torch.autograd.profiler.record_function("EnsembleModel: forward_decoder"):
                ...
                lprobs, avg_attn_scores = self.model.forward_decoder(
                        ...
                        self.temperature,
                        new_sent=new_sent,
                        newly_finished=self.newly_finished,
                        beam_ids=self.beam_ids,
                        example_idx=self.idx,
            ...
            self.newly_finished=finalized_sents
        ...
        for i, model in enumerate(self.models):
            ...
            if self.has_incremental_states():
                ...
                if hasattr(model, "decoder"):
                    decoder_out = model.decoder.forward(tokens, new_sent=new_sent, newly_finished=newly_finished, encoder_out=encoder_out, beam_ids=beam_ids, example_idx=example_idx, tgt_dict=tgt_dict)
                else:
                    decoder_out = model.forward(tokens, new_sent=new_sent,  newly_finished=newly_finished, beam_ids=beam_ids, example_idx=example_idx, tgt_dict=tgt_dict)
